...
cam_states = np.zeros((len(cam_data)-3,16))

for j in range(3, len(cam_data)):
    cam_states[j-3] = cam_data[j].get('camera_to_world')
...
with open(os.path.join(script_dir, 'camera_path_spline_new.json'),'w+') as f:
    json.dump(res, f)
